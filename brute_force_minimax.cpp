...
Move minimax(int world[][MAX], int sum[], int depth, int turn);
...
Move minimax(int world[][MAX], int sum[], int depth, int turn){//pos starts from 0!!
    Move best;
    ...
    vector<int> avail;
    int new_score;
    int minimizing = depth % 2;//0 if even, 1 if odd
    ...
    for(unsigned int i = 0; i < avail.size(); i++){
        ...
        new_score = minimax(world, sum, depth + 1, turn).score;
